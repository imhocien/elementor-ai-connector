...
class EMCP_Tools_Query_Abilities {
			...
			array(
				...
				'input_schema'        => array(
					...
					'properties' => array(
						...
						'live'     => array(
							'type'        => 'boolean',
							'description' => __( 'When true, list widgets from Elementor\'s live registry instead of the curated catalog. Use this to discover third-party addon widgets.', 'emcp-tools' ),
						),
					),
				),
				'output_schema'       => array(
	...
	public function execute_list_widgets( $input = null ): array {
		...
		if ( $live && class_exists( '\Elementor\Plugin' ) ) {
			return $this->execute_list_widgets_live( $tier, $category, $search );
		}
	...
	/**
	 * Lists widgets from Elementor's live widget registry.
	 *
	 * This discovers ALL registered widget types, including those from
	 * third-party addon packs (Essential Addons, Premium Addons, UAE, etc.).
	 *
	 * @since 3.7.0
	 *
	 * @param string $tier    Filter (unused for live, kept for API compat).
	 * @param string $category Filter by widget category.
	 * @param string $search   Search term.
	 * @return array The widgets list.
	 */
	private function execute_list_widgets_live( string $tier, string $category, string $search ): array {
		...
		foreach ( $all_widgets as $name => $widget ) {
			...
			if ( '' !== $search_lower ) {
				$title = strtolower( $widget->get_title() );
				if ( ! str_contains( $title, $search_lower ) && ! str_contains( strtolower( $name ), $search_lower ) ) {
					continue;
				}
			}

			$controls   = $widget->get_controls();
			$param_names = array_keys( $controls );
			$cats        = $widget->get_categories();
			...
			$rows[] = array(
				...
				'tier'        => 'live',
				'category'    => $cat_str,
				'use_case'    => $widget->get_description(),
		...
		emcp_tools_register_ability(
			...
			array(
				...
				'input_schema'        => array(
					...
					'live'        => array(
						'type'        => 'boolean',
						'description' => __( 'When true, look up the widget from Elementor\'s live registry instead of the curated catalog. Use for third-party widgets not in the catalog.', 'emcp-tools' ),
					),
				),
				),
				'output_schema'       => array(
	...
	public function execute_get_widget_schema( $input ) {
		...
		$build = function ( $type ) use ( $full, $live ) {
			if ( $live && class_exists( '\Elementor\Plugin' ) ) {
				return $this->execute_get_widget_schema_live( $type );
			}
			...
			if ( null === $entry ) {
				return array(
					'widget_type' => $type,
					'error'       => __( 'Not in the curated catalog. Retry with full:true or live:true for the raw control schema.', 'emcp-tools' ),
				);
			}
	...
	/**
	 * Gets widget schema from Elementor's live registry.
	 *
	 * Returns the full control schema for any registered widget, including
	 * third-party addon widgets not in the curated catalog.
	 *
	 * @since 3.7.0
	 *
	 * @param string $type The widget type name.
	 * @return array The widget schema.
	 */
	private function execute_get_widget_schema_live( string $type ): array {
		$widgets_manager = \Elementor\Plugin::$instance->widgets_manager;
		...
		if ( ! $widget ) {
			return array(
				'widget_type' => $type,
				'error'       => __( 'Widget type not found in Elementor\'s registry.', 'emcp-tools' ),
			);
		}
	...
	private function register_list_templates(): void {
		emcp_tools_register_ability(
			...
			array(
				...
				'description'         => __( 'Returns all saved Elementor templates from the template library. Optionally filter by template type (page, section, container) or search by title keyword.', 'emcp-tools' ),
				'category'            => 'emcp-tools',
				...
				'input_schema'        => array(
					...
					'search'        => array(
						'type'        => 'string',
						'description' => __( 'Search by keyword in template title.', 'emcp-tools' ),
					),
				),
				),
				'output_schema'       => array(
