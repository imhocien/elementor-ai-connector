...
class EMCP_Tools_Yoast_Integration extends EMCP_Tools_SEO_Integration {
	...
	protected function operations(): array {
		...
		return array(
			'get-post-seo'    => array(
				...
				'perm' => $edit_posts,
				'desc' => 'Get a post\'s Yoast SEO metadata by { post_id } (title, description, canonical, noindex, nofollow, og_image, twitter_image, focus_keyword).',
			),
			'get-term-seo'    => array(
				'mode' => 'read',
				'run'  => array( $this, 'op_get_term_seo' ),
				'perm' => $edit_posts,
				'desc' => 'Get a term\'s Yoast SEO metadata by { term_id }.',
			),
			'get-settings'    => array(
				'mode' => 'read',
				'run'  => array( $this, 'op_get_settings' ),
				'perm' => $manage,
				'desc' => 'Get Yoast SEO site settings.',
			),
			'update-post-seo' => array(
				'mode' => 'write',
				'run'  => array( $this, 'op_update_post_seo' ),
				'perm' => $edit_posts,
				'desc' => 'Update a post\'s Yoast SEO metadata: { post_id, title?, description?, canonical?, noindex?, nofollow?, og_image?, twitter_image?, focus_keyword? }. Only provided fields change.',
			),
			'update-term-seo' => array(
				'mode' => 'write',
				'run'  => array( $this, 'op_update_term_seo' ),
				'perm' => $edit_posts,
				'desc' => 'Update a term\'s Yoast SEO metadata: { term_id, title?, description?, ... }.',
			),
		);
	}
	...
	private function map(): array {
		return array(
			...
			'noindex'        => '_yoast_wpseo_noindex',
			'nofollow'       => '_yoast_wpseo_nofollow',
